...
# these four checks are things that are TRUE OR FALSE about a live deployment,
...
# 2. Auth is wired and fails CLOSED. An unauthenticated call to a tenant route
...
# 3. The agent surface refuses an unauthenticated write.
...
# 4. The security headers added in Phase 4 survived deployment. Easy to lose to
...
echo
if [ "$FAILED" = "0" ]; then
  echo "=== SMOKE RESULT: PASS (4/4) ==="
else
  echo "=== SMOKE RESULT: FAIL ($FAILED of 4) ==="
fi
exit $FAILED
